...
function Overview({ data, openCompany }) {
  ...
  return <>
    ...
    <section className="metrics-grid">
      <Metric label="Research universe" value={`${data.universeScope?.dashboardCompanies || data.companies.length} / ${data.universeScope?.backtestCompanies || data.companies.length}`} hint="页面核心 / 回测样本" />
...
function Signals({ companies, openCompany }) {
  ...
  return <section>
    ...
    <div className="table-panel signal-table">
      <table><thead><tr><th>Rank</th><th>Company</th><th>产业链同组</th><th>Signal</th><th>Signal Δ</th><th>20D Excess</th><th>Rev YoY / Accel</th><th>EPS Rev 30D</th><th>FCF Margin YoY Δ</th><th>Valuation %ile</th><th>同层 Gap</th></tr></thead>
      <tbody>{filtered.map((company, index) => <tr key={company.ticker} onClick={() => openCompany(company.ticker)}>
        ...
        <td><SignalPill label={company.researchSetup}/><small className="action-label">F {company.fundamentalScore ?? '—'} · M {company.marketScore ?? '—'}</small></td>
    ...
    <p className="footnote">v2 同层 Gap = 多维基本面分数 − 市场分数。基本面由增长30%、增长加速25%、现金流质量25%、EPS预期修正20%构成；市场分数由20D相对表现60%和60D相对表现40%构成。缺失项不会记为0分，而会重分配可用权重并展示覆盖率。正值只代表值得研究的背离，不自动等于低估。</p>
  </section>
}
...
function FactorLab({ hypotheses }) {
  ...
  const cards = lab[tab] || []
  const metricText = (key, value, scope) => value == null ? '—' : key === 'sample_size' && scope?.rebalance_periods ? `${scope.rebalance_periods}期 / ${Math.round(value)}观测` : key === 'sample_size' ? Math.round(value) : key === 'rank_ic' ? num(value, 3) : pct(value)
    ...
    <div className="factor-lab-grid">{cards.map((card, cardIndex) => {
      ...
      return <article className={`factor-experiment ${cardIndex === 0 ? 'featured' : ''}`} key={card.experiment_id}>
        ...
        {card.factor_definition && <div className="factor-definition">
          <div className="factor-formula"><span>FACTOR CONSTRUCTION</span><b>{card.factor_definition.formula}</b>{card.factor_definition.backtest_formula && <small>{card.factor_definition.backtest_formula}</small>}<p>{card.factor_definition.question}</p></div>
...
function CompanyDrawer({ company, series, close }) {
  ...
  return <div className="drawer-backdrop" onClick={close}><aside className="drawer" onClick={e => e.stopPropagation()}>
    ...
    <div className="setup-card why-card"><span className="eyebrow">WHY FLAGGED? · FUNDAMENTAL V2</span><h2>{company.researchSetup}</h2><div className="why-metrics"><span>基本面得分<b>{company.fundamentalScore == null ? '—' : `${company.fundamentalScore}/100`}</b></span><span>市场得分<b>{company.marketScore == null ? '—' : `${company.marketScore}/100`}</b></span><span>定价错位 Gap<b className={tone(company.fundamentalPriceGap)}>{company.fundamentalPriceGap == null ? '—' : `${company.fundamentalPriceGap > 0 ? '+' : ''}${company.fundamentalPriceGap}`}</b></span><span>20D excess<b className={tone(company.excess20d)}>{pct(company.excess20d)}</b></span><span>60D excess<b className={tone(company.excess60d)}>{pct(company.excess60d)}</b></span></div><div className="score-breakdown">{[['growth','增长 · 30%'],['acceleration','增长加速 · 25%'],['cash_quality','现金流质量 · 25%'],['expectations','预期修正 · 20%']].map(([key,label]) => <span key={key}><small>{label}</small><b>{company.fundamentalComponents?.[key] == null ? '—' : company.fundamentalComponents[key]}</b></span>)}<span><small>数据覆盖</small><b>{pct(company.fundamentalScoreCoverage,0)}</b></span></div><p className="why-conclusion"><b>结论</b>{company.flagConclusion}</p>{company.latestCatalyst && <div><b>今日催化</b><span>{company.latestCatalyst}</span></div>}</div>
    ...
    <div className="drawer-section percentile-panel"><div className="panel-head"><div><span className="eyebrow">WITHIN-STAGE POSITION</span><h2>同层横截面分位</h2></div><span className="subtle">同层样本 {company.peerGroupSize || '—'} 家</span></div><PercentileBar label="基本面综合" value={company.fundamentalPercentile}/><PercentileBar label="相对估值吸引力" value={company.valuationPercentile}/><PercentileBar label="20D / 60D 市场综合" value={company.marketCompositePercentile}/></div>
